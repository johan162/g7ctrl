 ...
 * SVN:         $Id: unicode_tbl.c 766 2015-03-05 07:22:26Z ljp $
 *
 ...
 */
char *
get_txt(table_t *t, int row, int col) {
    if (_rc_chk(t, row, col))
        return NULL;
...
set_cell(table_t *t, size_t row, size_t col, char *val) {
    if (_rc_chk(t, row, col) || t->c[TIDX(row,col)].merged )
        return -1;
...
_draw_cellcontent_row(table_t *t, size_t row, tblstyle_t style) {
    ...
    while (c < t->nCol) {
        ...
        const int len_half = strlen(txt) / 2;
...
main(int argc, char **argv) {
    ...
    set_cell(tbl, 0, 0, "The very first table");
