...
unsigned minimap_overview_zoom;
unsigned minimap_detailed_zoom;
unsigned minimap_width;
unsigned minimap_height;

unsigned geocache_address_size;
unsigned geocache_minimap_size;

_Bool use_short_devid ;

_Bool pdfreport_geoevent_newpage ;
_Bool pdfreport_hide_empty_geoevent ;
char pdfreport_dir[1024] ;
 ...
 */
#define INIT_INISTR(KEY,VAR,DEF) {strncpy(VAR, iniparser_getstring(inifile_dict, KEY, DEF), sizeof(VAR)-1); VAR[sizeof(VAR)-1] = '\0';}
#define INIT_INIBOOL(KEY,VAR,DEF) {VAR = iniparser_getboolean(inifile_dict,KEY, DEF);}
#define INIT_INIINT(KEY,VAR,DEF,MINV,MAXV) {VAR = validate(MINV,MAXV,KEY,iniparser_getint(inifile_dict, KEY, DEF));}
#define INIT_INIDOUBLE(KEY,VAR,DEF,MINV,MAXV) {VAR = validate_double(MINV,MAXV,KEY,iniparser_getdouble(inifile_dict, KEY, DEF));}

void
close_inifile(void) {
    if( inifile_dict ) {
        iniparser_freedict(inifile_dict);
        inifile_dict=NULL;
    }    
}
...
setup_inifile(void) {
    ...
    if (*daemon_config_file) {
        ...
        inifile_dict = iniparser_load(daemon_config_file);
    } else {
...
read_inisettings(void) {
    ...
    INIT_INIDOUBLE("report:min_battery_voltage", min_battery_voltage, DEFAULT_MIN_BATT_VOLTAGE, 3.4, 3.7);
    INIT_INIDOUBLE("report:max_battery_voltage", max_battery_voltage, DEFAULT_MAX_BATT_VOLTAGE, 3.8, 4.3); 
