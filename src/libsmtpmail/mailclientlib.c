 ...
 * @param sfd File descriptor to read
...
_smtp_connect(char *server_name_or_ip, char *service) {
    ...
    ssize_t num = _smtp_split_reply(buffer, reply_list, MAX_REPLYLIST_LEN);
    free(buffer);

    if (1 != num) {
        shutdown(sock, SHUT_RDWR);
        close(sock);
        _free_smtplist(reply_list, MAX_REPLYLIST_LEN);
        return NULL;
    }
...
_smtp_normalize_mailaddr(char *mailaddr, char *name, size_t maxnlen, char *addr, size_t maxalen) {
    ...
    if (NULL == (left = strchr(mailaddr, '<')) ||
        ...
        if (NULL == strchr(tmp, ' ') && strchr(tmp, '@')) {
            ...
            snprintf(tmp2, strlen(tmp) + 3, "<%s>", tmp);
            if (maxalen < strlen(tmp2) + 1) {
                free(tmp);
                free(tmp2);
                return -1;
            }
            xstrlcpy(addr, tmp2, maxalen);
...
/**
 * Debug print routine. Dump all information about an SMTP session to
 * specified buffered file
 * @param handle SMTP session handle
 ...
 * Multiple recipients can be added by separating them with a ','
 * @param handle SMTP session handle
 * @param type
 * @param rcpt
 * @return 0 on success, -1 on failure
...
smtp_add_rcpt(struct smtp_handle *handle, unsigned type, char *rcpt) {
    ...
    if (-1 == xstrsplitfields(rcpt, ',', &sfields)) {
        return -1;
    }

    for (size_t i = 0; i < sfields.nf; ++i) {
        char email_namepart[127];
        char email_addrpart[127];
        char email_full[256];
        ...
        if (-1 == _smtp_normalize_mailaddr(sfields.fld[i], email_namepart, sizeof (email_namepart) - 1, email_addrpart, sizeof (email_addrpart) - 1)) {
            return -1;
        }

        // logmsg(LOG_DEBUG,"Normalize email RCPT: '%s' as ['%s', '%s']",rcpt,email_namepart,email_addrpart);

        snprintf(email_full, sizeof (email_full) - 1, "%s %s", email_namepart, email_addrpart);
        switch (type) {
            case SMTP_RCPT_TO:
                ...
                if (*handle->to_concatenated) {
                    xstrlcat(handle->to_concatenated, ",", MAX_HEADER_ADDR_SIZE);
                }
                xstrlcat(handle->to_concatenated, email_full, MAX_HEADER_ADDR_SIZE);
                break;
            case SMTP_RCPT_CC:
                ...
                if (*handle->cc_concatenated) {
                    xstrlcat(handle->cc_concatenated, ",", MAX_HEADER_ADDR_SIZE);
                }
                xstrlcat(handle->cc_concatenated, email_full, MAX_HEADER_ADDR_SIZE);
            ...
            case SMTP_RCPT_BCC:
                ...
                if (*handle->bcc_concatenated) {
                    xstrlcat(handle->bcc_concatenated, ",", MAX_HEADER_ADDR_SIZE);
                }
                xstrlcat(handle->bcc_concatenated, email_full, MAX_HEADER_ADDR_SIZE);
                break;
        }
    }
    return 0;
...
smtp_add_attachment_fromfile(struct smtp_handle *handle, char *filename, unsigned contenttype, unsigned encoding) {
    ...
    if ((fp = fopen(filename, "rb")) == NULL) {
        logmsg(LOG_ERR, "Cannot open attachment file \"%s\" ( %d : %s )", filename, errno, strerror(errno));
        return -1;
    }
    int fd = fileno(fp);
    if (-1 == fd) {
        logmsg(LOG_ERR, "Cannot get fileno for attachment file \"%s\" ( %d : %s )", filename, errno, strerror(errno));
        fclose(fp);
        return -1;
    }
    struct stat buf;
    if (-1 == fstat(fd, &buf)) {
        logmsg(LOG_ERR, "Cannot get filestat for attachment file \"%s\" ( %d : %s )", filename, errno, strerror(errno));
        fclose(fp);
        return -1;
    }
    ...
    if (readsize != (size_t) buf.st_size) {
        logmsg(LOG_ERR, "Read file size (=%zu) is different from actual file size (=%zu)", readsize, (size_t) buf.st_size);
        fclose(fp);
        free(buffer);
        return -1;
    }
    ...
    // We need a temporary filename in case basename() modifies its argument
    char *tmp_filename = strdup(filename);
    int rc = smtp_add_attachment(handle, basename(tmp_filename), basename(tmp_filename), buffer, buf.st_size, contenttype, encoding);
    free(tmp_filename);
    free(buffer);
    return rc;
}
...
int
get_imagecontenttype_from_string(const char *imageformat) {
    
    if (strcasecmp(imageformat, "jpg") == 0 || strcasecmp(imageformat, "jpeg") == 0) {
        return SMTP_ATTACH_CONTENT_TYPE_JPG;
    } else if (strcasecmp(imageformat, "png") == 0) {
        return SMTP_ATTACH_CONTENT_TYPE_PNG;
    } else if (strcasecmp(imageformat, "gif") == 0) {
        return SMTP_ATTACH_CONTENT_TYPE_GIF;
    } else {
        return -1;
    }    
}
...
/**
 * Adds an attachment in the mail that holds one inline image that is referenced in the HTML
 * section of the mail as img href="cid:XX" where XX is the cid specified as the last argument
 * to the function. This should be a unique ID within the mail.
 * @param handle SMTP session handle
 * @param filename
 * @param cid
 * @return -1 on failure, 0 on success
...
smtp_add_attachment_inlineimage(struct smtp_handle *handle, char *filename, char *cid) {
    ...
    int contenttype = get_imagecontenttype_from_string(ebuff);
    if( -1 == contenttype ) return -1;
...
/**
 * Add an inline image where the image data already exist in a memory buffer
 * @param handle The smtp handle
 * @param imgfilename  The image name included in the attachment
 * @param imagedata Image data buffer
 * @param len Length of image data
 * @param imgformat Format of image as a string "jpg", "png", "gif"
 * @param cid The content id for the image to be used in the referencing HTML
 * @return -1 on failure, 0 on success
 */
int
smtp_add_attachment_inlineimage_buffer(struct smtp_handle *handle, char *imgfilename, char *imagedata, size_t len, 
        char *imgformat, char *cid) {
    
    int contenttype = get_imagecontenttype_from_string(imgformat);
    if( -1 == contenttype ) 
        return -1;
    
    int rc = smtp_add_attachment(handle, imgfilename, imgfilename, imagedata, len, contenttype, SMTP_CONTENT_TRANSFER_ENCODING_BASE64);
...
/**
 * Adds an attachment in the mail for a gzipped file which are encoded in BASE64
 * @param handle SMTP session handle
 * @param filename
 * @return
 */
...
smtp_add_attachment_gzip(struct smtp_handle *handle, char *filename) {
    ...
    if (-1 == smtp_add_attachment_fromfile(handle, filename, SMTP_ATTACH_CONTENT_TYPE_GZIP, SMTP_CONTENT_TRANSFER_ENCODING_BASE64)) {
        return -1;
    }
...
/**
 * Adds an attachment in the mail for a binary file which are encoded in BASE64
 * @param handle SMTP session handle
 * @param filename
 * @return
 */
...
smtp_add_attachment_binary(struct smtp_handle *handle, char *filename) {
    ...
    if (-1 == smtp_add_attachment_fromfile(handle, filename, SMTP_ATTACH_CONTENT_TYPE_OCTET, SMTP_CONTENT_TRANSFER_ENCODING_BASE64)) {
        return -1;
    }
...
smtp_sendmail(struct smtp_handle *handle, char *from, char *subject) {
    ...
    if (handle->attachmentidx == 0) {
        ...
        if (handle->html == NULL) {
            ...
            char boundary[255];
            snprintf(boundary, 255, "_%x%x%x%x%s_", (unsigned)rand(), (unsigned)rand(), (unsigned)rand(), (unsigned)rand(), hname);
        ...
        size_t databufflen=0;
        if (handle->plain && handle->html == NULL) {
            // A plain text mail
            databufflen = strlen(handle->plain) + adbsize + 512;
            handle->databuff = calloc(1, databufflen);
            snprintf(handle->databuff, databufflen,
                    "--%s\r\n"
                    "Content-Transfer-Encoding: 8bit\r\n"
            ...
            char boundary2[255];
            snprintf(boundary2, 255, "_%x%x%x%x%s_", (unsigned)rand(), (unsigned)rand(), (unsigned)rand(), (unsigned)rand(), hname);

            databufflen = strlen(handle->html) + strlen(handle->plain) + adbsize + 512;
            handle->databuff = calloc(1, databufflen);
            snprintf(handle->databuff, databufflen,
                    "--%s\r\n"
        ...
        for (size_t i = 0; i < handle->attachmentidx; ++i) {
            ...
            xstrlcat(handle->databuff, tmpbuff, databufflen);
            free(tmpbuff);
        }
        char boundbuff[512];
        snprintf(boundbuff, 512, "--%s--\r\n", boundary);
        xstrlcat(handle->databuff, boundbuff, databufflen);
...
/**
 * This is the opposite to smtp_setup. This will cleanup all used memory by the
 * smtp session
 * @param handle SMTP session handle
 */
void
 ...
 * @param isHTML
 * @param filename
 * @param contenttype if specified as -1 then assume plain type
