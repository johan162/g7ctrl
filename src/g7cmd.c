...
struct g7event g7event_list[] = {
    ...
    {EVENT_TRACK, "Position update", "TRACK"},
    ...
    { "ps", 7,
        {
            {"Mode", "When to enable sleep mode", ARGT_SELECT, 4,
                {
                    {"0", "Auto sleep. Wake on movement. GSM=Stdby,GPRS=GPS=Off,G=On"},
                    {"1", "Auto sleep. Wake every n min. GSM=GPRS=GPS=G=Off"},
                    {"2", "Auto sleep. Wake on timer. GSM=GPRS=GPS=G=Off"},
                    {"3", "Auto sleep. Wake on movement. GSM=GPRS=GPS=Off,G=On"}
                }},
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_INVALID_GET "[ERR] Invalid invocation of \"%s\""
static int
cmd_get_template(struct client_info *cli_info, const int cmdidx, const int gsflag) {
    ...
    *buff = '\0';
    if (gsflag) {
        _writef(sockd, ERR_CMD_INVALID_GET, g7command_list[cmdidx].cmdstr);
        return -1;
    }
    return prepare_cmd_to_device(cli_info, cmdidx, mode, buff);
}
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_INTERNAL_SET "Internal error in cmd_set_template(). Negative argidx"
#define ERR_CMD_GFEVT "[ERR] Missing argument. Command synopsis: \"get gfevt EVENTID\""
static int
    ...
    *buff = '\0';
    ...
    switch (gsflag) {
        ...
        case 1: // On
            ...
            int argidx = get_argidx_for_cmdidx(cmdidx);
            if (argidx < 0) {
                _writef(sockd, "[ERR] %s", ERR_CMD_INTERNAL_SET);
                logmsg(LOG_ERR, "%s", ERR_CMD_INTERNAL_SET);
                return -1;
            }
            if (0 == cmdargs_list[argidx].numargs) {
                xstrlcpy(buff, "1", sizeof(buff));
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_INVALID_INVOCATION "[ERR] Invalid invocation of \"%s\""
static int
cmd_do_template(struct client_info *cli_info, const int cmdidx, const int gsflag) {
    ...
    char buff[16] = {'\0'};
    if (gsflag) {
        _writef(sockd, ERR_CMD_INVALID_INVOCATION, g7command_list[cmdidx].cmdstr);
        return -1;
    }
    return prepare_cmd_to_device(cli_info, cmdidx, mode, buff);
}
 ...
 * @return -1
 */
#define ERR_CMD_NOT_SUPPORTED "[ERR] Not supported command. Internal fault.\n"
static int
error_notsupported_cmd(const int sockd, const char *cmd, int arg) {
    _writef(sockd, ERR_CMD_NOT_SUPPORTED);
 ...
 * @return -1 on failure , 0 on success
 */
#define ERR_CMD_NOT_VALID "[ERR] This command is not valid. See \"help %s\"\n"
int
prepare_cmd_to_device(struct client_info *cli_info, const int cmdidx, const int mode, const char *argstrlist) {
    ...
    if (!(mode & g7command_list[cmdidx].modes)) {
        _writef(sockd, ERR_CMD_NOT_VALID, g7command_list[cmdidx].srvcmd);
 ...
 * @return -1 on failure, 0 on success
 */
#define ERR_CMD_INPUT_ABORTED "[ERR] Input aborted."
int
get_cmd_args(const int sockd, int cmdidx, size_t maxlen, char *argbuff) {
    ...
    *argbuff = '\0';
    for (size_t i = 0; i < p->numargs; i++) {
                ...
                if (0 == strcmp(AUTO_ENABLE_LABEL, p->argl[i].arglabel)) {
                    ...
                    do {
                        ...
                        if (1 <= strlen(reply) && '.' == *reply) {
                            _writef(sockd, ERR_CMD_INPUT_ABORTED);
                            return -1;
                        }
        ...
        if (i > 0)
            xstrlcat(argbuff, ",",maxlen);
        xstrlcat(argbuff, reply,maxlen);
    }
 ...
 * @return 0 on success, -1 on failure
 */
#define GOOGLE_MAP_URL "https://www.google.com/maps?q=%s,%s"
int
get_googlemap_string(char *lat, char *lon, char *url, size_t maxlen) {

    *url = '\0';
    if (8 <= strlen(lat) && 8 <= strlen(lon)) {
        snprintf(url, maxlen, GOOGLE_MAP_URL, lat, lon);
        return 0;
    } else {
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_INTERNAL_EXECUTE_COMMAND "[ERR] Internal error cannot execute command"
#define ERR_CMD_GETLOC "[ERR] Failed to get location"
int
exec_get_locg(struct client_info *cli_info) {
    const int sockd = cli_info->cli_socket;
    char reply[256];
    if (-1 == send_cmdquery_reply(cli_info, "locg", reply, sizeof (reply))) {
        _writef(sockd, ERR_CMD_INTERNAL_EXECUTE_COMMAND);
        return -1;
    }
    // 3:rd and 4:th fields are the lon & lat so extract them from the device reply
    struct splitfields flds;
 ...
 */
#define ERR_CMD_INVALID_EVENTID "[ERR] Event ID must be in range [50,99]"
int
exec_get_gfevt(struct client_info *cli_info, char **field) {
    ...
    if (evtid < 50 || evtid > 99) {
        _writef(sockd, ERR_CMD_INVALID_EVENTID);
        return -1;
    }
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_ADDRESS "[ERR] Failed to get address"
int
 ...
 * @return 0 on success, -1 on failure
 */
#define ERR_CMD_BATTVOLT "[ERR] Failed to read battery voltage."
...
exec_cmd_batt(struct client_info *cli_info, const int cmdidx, const int gsflag) {
    ...
    if (!rc) {
        ...
        _writef(sockd, ERR_CMD_BATTVOLT);
    }
    return rc;
}
 ...
 */
#define ERR_CMD_NATIVE_NOT_ENABLED "[ERR] Native command sending NOT enabled by administrator."
...
exec_native_command(struct client_info *cli_info, const char *cmdstr) {
    ...
    if (enable_raw_device_commands) {
        ...
        _writef(sockd, ERR_CMD_NATIVE_NOT_ENABLED);
    }
}
...
check_password(int sockd) {
    static const char *AUTH_FAIL_MSG = INVALID_AUTHENTICATION;
...
//        rc = exec_get_address(cli_info);        
        ...
        char *tmpField[3];
        tmpField[0] = (char *) calloc(32, sizeof(char));
        tmpField[1] = (char *) calloc(32, sizeof(char));
        tmpField[2] = (char *) calloc(32, sizeof(char));
...
cmd_clientsrv(void *arg) {
    ...
    logmsg(LOG_DEBUG, "NEW Client at cli_socket=%d", cli_info->cli_socket);    
    
    if( -1 == check_password(cli_info->cli_socket) ) {
        logmsg(LOG_ERR, "Failed password check for command login");    
        goto close_cmd_socket;        
    }
    

#ifdef __APPLE__
    _writef(cli_info->cli_socket, "GM7 Server version %s \nAutomatic logout after %d minutes of idle time.\nType \"help\" to list available commands.", 
            PACKAGE_VERSION, max_idle_time / 60);
#else
    _writef(cli_info->cli_socket, "GM7 Server version %s (build %lu-%lu)\nAutomatic logout after %d minutes of idle time.\nType \"help\" to list available commands.",
            PACKAGE_VERSION, (unsigned long) &__BUILD_DATE, (unsigned long) &__BUILD_NUMBER, max_idle_time / 60);
#endif
