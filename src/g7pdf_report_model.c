...
/*
 * Keys for the device status dictionary where the device status is collected 
 */
...
extract_logged_num_and_dates(struct splitfields *flds, size_t idx) {
    ...
    *nptr='\0';
    if( '(' == *ptr ) {
        ...
        // it more readable and a hyphen in the dates and colon between h,m and s
        ptr++;
        dptr=dates;      
        int cnt=0;
        while( *ptr && *ptr!=')' ) {
 ...
 */
int
init_model_from_device(void *tag, _Bool read_from_file, _Bool include_geofevt) {
    ...
    if( read_from_file ) {
        ...
        if( 0 == assoc_import_from_json_file(device_info,path) ) {
            logmsg(LOG_INFO,"Read device data from \"%s/export.json\"",pdfreport_dir);
            _writef(cli_info->cli_socket,"Reading data from file \"%s\"\n",path);
            return -0;
        } else {
            logmsg(LOG_ERR,"Failed to read device data from \"%s/export.json\" (%d : \"%s\")",pdfreport_dir,errno,strerror(errno));
            _writef(cli_info->cli_socket,"[ERR] Cannot read device data from file \"%s\"\n",path);
            return -1;            
        }
    ...
    while( dev_report_cmd_list[i].cmd_name ) {
        ...
        if( 0==strcmp("test",dev_report_cmd_list[i].cmd_name) ) {
            usleep(2000000); // Sleep two seconds to wait for result from test
        } else {
            usleep(500000); // Sleep 0.5 seconds between two commands
        }
    ...
    if (include_geofevt) {
        ...
        char cmdbuf[32];
        char namebuf[64];
        // The events are between 50 to 99 and we have to check each one
    ...
    time_t t = time(NULL);
    ctime_r(&t,date_buf);
    date_buf[strlen(date_buf)-2]='\0'; // Get rid of the trailing '\n'
...
cb_POWER_WAKEUP(void *tag, size_t r, size_t c) {
    struct client_info *cli_info = (struct client_info *)tag;
    static char buf[16];
    snprintf(buf,sizeof(buf),"%s,%s",GET(FLD_PS_WAKEUP_ACTION),GET(FLD_PS_VIP));
    return buf;    
}
...
cb_BATTERY_PERCENT(void *tag, size_t r, size_t c) {
    // Maximum voltage is 4.2 and battery low warning is triggered at 3.7
    // We therefore assume that the possible range for the battery is 3.6 - 4.2
    // where 3.6 corresponds to 0% and 4.2 to 100% 
    // However , these settings are adjustable in the configuration file
    const struct client_info *cli_info = (struct client_info *)tag;
    const double volt = atof(GET(FLD_TEST_BATT));
    logmsg(LOG_DEBUG,"Min Batt=%.1f, Max batt=%.1f, Volt=%.1f",min_battery_voltage,max_battery_voltage,volt);
    return (float)((volt-min_battery_voltage)/(max_battery_voltage-min_battery_voltage));
}
...
/**
 * Translate a date in format "2015-01-31 19:46" to "31 Jan 2015, 19:46"
 * Uses English as locale
 * @param date
 * @return New date string as a pointer to static storage (not thread safe)
 */
static char *
_reformat_date(char const *date) {
    static char* months[] = {
      "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
    };
    static char fdate[32];
    char year[5], day[3], month[3], time[10];
    
    xstrlcpy(year,date,sizeof(year));
    xstrlcpy(month,date+5,sizeof(month));
    xstrlcpy(day,date+8,sizeof(day));
    xstrlcpy(time,date+11,sizeof(time));
    time[5] = '\0'; // Ignore seconds
    
    unsigned d = xatoi(day);    
    unsigned m = xatoi(month)-1;
    if( m > 11 ) {
        m=0;
        logmsg(LOG_ERR,"Month number out of range in report logging dates");
    }
   
    snprintf(fdate,sizeof(fdate),"%u %s %s, %s",d,months[m],year,time);
    return fdate;
}

/**
 * Table cell value callback to get the start date for the stored locations
 * on the device
 * @param tag Table tag
 * @param r Row where value will be place
 * @param c Column where value will be place
 ...
 */
char *
cb_LOGGED_dates_start(void *tag, size_t r, size_t c) {
    struct client_info *cli_info = (struct client_info *)tag;
    static char start[32];
    xstrlcpy(start,GET(FLD_LOGGED_DATES),20); // Only copy the first start date
    return _reformat_date(start);
}


/**
 * Table cell value callback to get the end date for the stored locations
 * on the device
 * @param tag Table tag
 * @param r Row where value will be place
 * @param c Column where value will be place
 ...
 */
char *
cb_LOGGED_dates_end(void *tag, size_t r, size_t c) {
    struct client_info *cli_info = (struct client_info *)tag;
    char buf[64];
    static char end[32];
    xstrlcpy(buf,GET(FLD_LOGGED_DATES), sizeof(buf));    
    xstrlcpy(end,&buf[22],20); // Only copy the ending date
    return _reformat_date(end);
}
