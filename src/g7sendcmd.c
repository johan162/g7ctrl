 ...
 * SVN:         $Id: g7sendcmd.c 855 2015-03-19 17:22:30Z ljp $
 *
...
cmdqueue_insert(const int user_sockd, const unsigned devid, const char *tag, const char *cmdstr) {
    ...
    for (size_t i = 0; i < MAX_CMDQUEUE_LEN; ++i) {
        if (0 == cmdq[i].devid && 0 == cmdq[i].ts) {
            ...
            strcpy(cmdq[i].tag, tag);
            strcpy(cmdq[i].cmd, cmdstr);
...
get_device_tag(char *tagbuff, const size_t maxlen) {
    ...
    if (tagnbr > 9999)
        tagnbr = 0;
    snprintf(tagbuff, maxlen, "%04u", tagnbr);
...
handle_device_reply(const int sockd, const char *reply) {
    ...
    if (rc) {
        ...
        if (!isok) {
            ...
            char resp[128];
            if (flds.nf > 0) {
                strcpy(resp, flds.fld[0]);
                for (size_t i = 1; i < flds.nf; i++) {
                    strcat(resp, ",");
                    strcat(resp, flds.fld[i]);
                }
            } else {
                strcpy(resp, "OK");
            }
