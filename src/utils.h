...
/**
 * Used to store a key/val pair. This is used for creating associative lists
 * to retrieve values from a given key. See get_assoc_value(),
 * get_assoc_value_s(), get_assoc_value_i()
 */
struct keypair_t {
    char key[255];
    char val[255];
...
int
get_assoc_value(char *value, size_t maxlen, char *key, char *list[], const size_t listlen);

int
get_assoc_value_s(struct keypair_t kv[],const size_t maxlen,char *key,char **v);

int
get_assoc_value_i(struct keypair_t kv[],const size_t maxlen,char *key,int *v);
