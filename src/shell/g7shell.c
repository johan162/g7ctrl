...
char *cmd_list[] = {
    ...
    ".lookup", ".table", ".nick", ".ln", ".dn", ".ratereset", ".report", ".breport", ".freport", 
    "exit", "quit",
    (char *) NULL
};
...
char *help_cmd_list[] = {
    ...
    ".ln", ".dn", ".ratereset", ".report", ".breport", 
    "address", "ver", "locg", "gfevt", "phone",
    "roam", "led", "gfen", "sleep", "loc",
    "imei", "sim", "ver", "nrec", "batt", "track", "mswitch", "tz",
    "sms", "comm", "vip", "ps", "config", "rec", "lowbatt", "sens",
    ...
    while (totlen < maxbufflen) {
        ...
        if (0 == rc) {
            ...
            if (totlen + nread < maxbufflen) {
                xstrlcat(buffer, pbuff, maxbufflen);
                totlen += nread;
            } else {
                free(pbuff);
...
setup_inifile(void) {
    ...
    close(STDERR_FILENO);
    inifile_dict = iniparser_load(inifile);
    dup2(STDOUT_FILENO,STDERR_FILENO);

    if (NULL == inifile_dict) {
       return -1;
    }
    return 0;
}
...
read_inifile(void) {
    if (inifile_dict) {
        if (tcpip_port == -1) {
            tcpip_port = iniparser_getint(inifile_dict, "config:cmd_port", DEFAULT_CMD_PORT);
        }
        strncpy(g7ctrl_pwd, iniparser_getstring(inifile_dict, "config:client_pwd", ""), 127);
        g7ctrl_pwd[127] = '\0';
        return 0;
    } else {
        return -1;
