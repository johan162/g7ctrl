 ...
 * SVN:         $Id: export.c 908 2015-04-03 23:24:03Z ljp $
 *
...
export_to_internal_set(sqlite3 *sqlDB, char *fromDate, char *toDate, char *deviceId, char *eventId) {
    ...
    *w = '\0';
    add_wcond(w, "fld_datetime", ">=", fromDate);
    add_wcond(w, "fld_datetime", "<=", toDate);
    add_wcond(w, "fld_deviceid", "=", deviceId);
    add_wcond(w, "fld_event", "=", eventId);
    strcat(q, w);
    strcat(q, ";");
...
mail_as_attachment(struct client_info *cli_info, char *exportFormat) {
    ...
    if (*attachment_compression == 'g')
        strcat(tmpExportFile, ".gz");
    else if (*attachment_compression == 'x')
        strcat(tmpExportFile, ".xz");
    else if (*attachment_compression == 'b')
        strcat(tmpExportFile, ".bz2");
    else {
    ...
    add_keypair(keys, max_keypairs, "FORMAT", exportFormat, &keyIdx);
    ...
    snprintf(subject, sizeof(subject), "%sExported database", mail_subject_prefix);

    rc = send_mail_template(subject, daemon_email_from, to, templatename, keys, keyIdx, max_keypairs, tmpExportFile, 0, NULL);
