...
callback_preset_file(char *fullPath, size_t seqNbr) {
    ...
    size_t llen = 512;
    char *lineBuffer = calloc(llen,sizeof(char));
        ...
        if (rc > 0) {
            ...
            if ('#' == *lineBuffer) {
                ...
                if (0 == lnbr) {
                    ...
                    xstrlcat(presets[seqNbr].longdesc, "\n",sizeof(presets[seqNbr].longdesc));
                }
            } else {
                // Command line
...
execPresetFunc(struct client_info *cli_info, const char *presetString, const char *tag, const char *pin) {
    ...
    *c = '\0'; // Terminate function name at the first '('
    ...
    do {
        ...
        *ap = '\0';
        if (*argBuf) {
            xstrlcat(cmdBuf, ",", sizeof(cmdBuf));
            xstrlcat(cmdBuf, argBuf, sizeof(cmdBuf));            
        } else {
